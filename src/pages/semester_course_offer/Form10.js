...
const Form10 = ({ formData, onSubmit }) => {
  ...
  const [tableData, setTableData] = useState([{...tableDataFormat, section: formData.section[0], marks_upload_right:"1"},{...tableDataFormat, section: formData.section[1], marks_upload_right:"1"},{...tableDataFormat, section: formData.section[2], marks_upload_right:"1"},{...tableDataFormat, section: formData.section[3], marks_upload_right:"1"}]);
  const handleAddRow = () => {
  ...
  return(
    <div>
      <button onClick={() => rel10(formData)}>Reset</button>
      <h2>IC Course Selection</h2>
            ...
            <tbody>
              {tableData.map((tada, index) => (
                <tr key={index}>
                  {index>=0 && index<4 &&(
                    <td>
                    <select value={tada.section} onChange={(e) => handleChangeRow(index, 'section', e.target.value)} required disabled>
                      <option value="">Select Section</option>
                      <option value={formData.section[0]}>{formData.section[0]}</option>
                      <option value={formData.section[1]}>{formData.section[1]}</option>
                      <option value={formData.section[2]}>{formData.section[2]}</option>
                      <option value={formData.section[3]}>{formData.section[3]}</option>
                    </select>
                    </td>
                  )}
                  {index>=4 &&(
                    <td>
                    <select value={tada.section} onChange={(e) => handleChangeRow(index, 'section', e.target.value)} required>
                      <option value="">Select Section</option>
                      <option value={formData.section[0]}>{formData.section[0]}</option>
                      <option value={formData.section[1]}>{formData.section[1]}</option>
                      <option value={formData.section[2]}>{formData.section[2]}</option>
